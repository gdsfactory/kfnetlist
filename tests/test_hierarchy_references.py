...
def _document():
    ...
    top = Netlist()
    top.create_inst("left", "pdk", "make_arm", {"length": 10.0}, ref="arm_10")
    top.create_inst("right", "pdk", "make_arm", {"length": 10}, ref="arm_10")
...
def test_legacy_and_explicit_construction():
    ...
    assert not hasattr(legacy, "ref")
    assert legacy.to_dict() == {
        "kcl": "pdk",
        "component": "straight",
        "settings": {"length": 10},
    }
    explicit = RefNetlistInstance("pdk", "make_arm", ref="arm_10")
...
def test_invalid_reference_never_falls_back_to_leaf(value):
    wire = {"kcl": "pdk", "component": "arm", "ref": value}
...
def test_explicit_loaders_reject_wrong_variant_and_unknown_fields():
    ...
    with pytest.raises(ValueError):
        LeafNetlistInstance.from_dict(leaf | {"ref": "arm_10"})
    with pytest.raises(ValueError):
        NetlistInstance.from_dict(leaf | {"ref": "arm_10", "typo": 1})
...
def test_hierarchy_roundtrip_shared_child_normalization_and_snapshots():
    ...
    assert normalized.instances["left"].ref == "arm_10"
...
def test_equality_includes_variant_and_reference():
    ...
    ref_a = RefNetlistInstance("pdk", "arm", ref="a")
    ref_b = RefNetlistInstance("pdk", "arm", ref="b")
...
def test_missing_target_and_cycles_fail_at_document_boundary():
    ...
    doc["arm_10"].create_inst("loop", "pdk", "top", ref="top")
...
def test_nested_references_flatten_without_side_maps():
    doc = _document()
    root = Netlist()
    root.create_inst("nested", "pdk", "make_top", ref="top")
    doc["root"] = root
...
def test_placed_conversion_preserves_logical_reference_separate_from_cell():
    ...
    assert PlacedInstance.from_dict(instance.to_dict()).ref == "arm_10"
    with pytest.raises(ValueError):
        PlacedInstance.from_dict(instance.to_dict() | {"ref": None})
...
def test_full_flatten_does_not_silently_return_unexpanded_references(empty_child):
    ...
    if empty_child:
        ...
        doc["top"].create_inst("array", "pdk", "make_arm", na=2, nb=3, ref="arm_10")
